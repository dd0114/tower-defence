// 간단한 WebSocket 서버 (Node.js 14 호환)
...
class SimpleWebSocketServer {
  ...
  handleFindMatch(clientId, playerData) {
    ...
    if (this.waitingPlayers.length >= 2) {
      ...
      console.log(`Game ${gameId} started with players: ${player1}, ${player2}`);
    } else {
  ...
  handlePlayerInput(clientId, inputData) {
    // 간단한 입력 처리 (실제 게임 로직은 클라이언트에서)
    console.log(`Player input from ${clientId}:`, inputData);
